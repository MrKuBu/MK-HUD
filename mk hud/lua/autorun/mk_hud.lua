...
if ply:Alive() then
	...
	surface.DrawTexturedRect(ScrW() / 20, ScrH() - 124, 400, 119)
	
	surface.DrawTexturedRect(ScrW() / 20, ScrH() - 124, 350, 119)
	
	surface.SetDrawColor(circlebg)
	draw.NoTexture()
	draw.Circle( ScrW() / 20, ScrH() - 64, 60,  70 )

	surface.SetDrawColor(circle)
	draw.NoTexture()
	draw.Circle( ScrW() / 20, ScrH() - 64, 55,  70 )
	...
	draw.SimpleText( name..": "..LocalPlayer():Nick()..", ".. job..": " ..LocalPlayer():getDarkRPVar('job'), "ImpactMKHUD", 128, ScrH() - 110, textcolor,  TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
	draw.SimpleText( money..": "..LocalPlayer():getDarkRPVar('money').." (+"..LocalPlayer():getDarkRPVar('salary')..")", "ImpactMKHUD", 143, ScrH() - 81, textcolor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
	
	local health_mul = math.Clamp( ply:Health() / ply:GetMaxHealth(), 0, 1 )
	local red_mod = 255*health_mul
	
	draw.RoundedBox( 9.4, 133, ScrH() - 60, ply:Health() *3, 19, Color(255-red_mod, 255*health_mul, 0, 255) ) 
	draw.RoundedBox( 9.4, 133, ScrH() - 60, 300, 19, Color(255-red_mod, 255*health_mul, 0, 130) )
	draw.SimpleText( health, "ImpactMKHUD", 240, ScrH() - 52, textcolor,  TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )

	if ply:Armor() > 0 then
		draw.RoundedBox( 10, 120, ScrH() - 30, ply:Armor()*3, 20, Color(0, 170, 255, 255) )
		draw.RoundedBox( 10, 120, ScrH() - 30, 300, 20, Color(0, 170, 255, 100) ) 
		draw.SimpleText( armor, "ImpactMKHUD", 240, ScrH() - 22, textcolor,  TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
	end
